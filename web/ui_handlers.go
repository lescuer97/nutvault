...
func SignerDashboard(serverData *ServerData) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		audience := mustAudience(r, w)
		if audience == nil {
			return
		}
		account, err := serverData.manager.AccountBelongsToPubkey(r.Context(), id, audience)
		if err != nil {
...
func UpdateAccountNameHandler(serverData *ServerData) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if err := serverData.manager.UpdateKeyName(r.Context(), account.Id, account.Name); err != nil {
...
func CertHandler(serverData *ServerData) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		case "key":
			...
			data, err := serverData.manager.GetTLSKey(account.Id)
			if err != nil {
...
func CertDownloadHandler(serverData *ServerData) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		case "key":
			d, err := serverData.manager.GetTLSKey(account.Id)
			if err != nil {
				http.Error(w, "key not found", http.StatusNotFound)
				return
			}
