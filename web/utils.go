...
func VerifyIdInRequestIsAvailable(serverData *ServerData, request *http.Request) (*database.Account, error) {
	...
	account, err := serverData.manager.AccountBelongsToPubkey(request.Context(), id, audPub)
	if err != nil {
		return nil, fmt.Errorf("serverData.manager.AccountBelongsToPubkey(...). %w", err)
	}
...
func mustAudience(r *http.Request, w http.ResponseWriter) *btcec.PublicKey {
