...
func DatabaseSetup(ctx context.Context, databaseDir string) (SqliteDB, error) {
	...
	db, err := sql.Open("sqlite3", databaseDir+"/"+"multi_account.db")
	if err != nil {
...
func (sq *SqliteDB) GetAllSeeds() ([]Seed, error) {
	seeds := []Seed{}
	stmt, err := sq.Db.Prepare(`SELECT  created_at, active, version, unit, id,  "input_fee_ppk", legacy, amounts, account_id, final_expiry FROM seeds ORDER BY version DESC`)
	if err != nil {
	...
	for rows.Next() {
		...
		amountsStr := ""
		timeUnix := int64(0)
		err = rows.Scan(&seed.CreatedAt, &seed.Active, &seed.Version, &seed.Unit, &seed.Id, &seed.InputFeePpk, &seed.Legacy, &amountsStr, &seed.AccountId, &timeUnix)
		if err != nil {
		...
		seed.FinalExpiry = time.Unix(timeUnix, 0)
...
func (sq *SqliteDB) GetSeedsByUnit(tx *sql.Tx, unit cashu.Unit) ([]Seed, error) {
	seeds := []Seed{}
	stmt, err := tx.Prepare("SELECT  created_at, active, version, unit, id, input_fee_ppk, legacy, amounts, account_id, final_expiry FROM seeds WHERE unit = $1")
	if err != nil {
