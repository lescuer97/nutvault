...
func (sq *SqliteDB) GetAllSeeds() ([]Seed, error) {
	seeds := []Seed{}
	stmt, err := sq.Db.Prepare(`SELECT  created_at, active, version, unit, id,  "input_fee_ppk", legacy FROM seeds ORDER BY version DESC`)
	if err != nil {
		return seeds, fmt.Errorf(`SELECT  created_at, active, version, unit, id,  "input_fee_ppk", legacy FROM seeds ORDER BY version DESC %w`, err)
	}
	defer stmt.Close()
	...
	for rows.Next() {
		var seed Seed
		err = rows.Scan(&seed.CreatedAt, &seed.Active, &seed.Version, &seed.Unit, &seed.Id, &seed.InputFeePpk, &seed.Legacy)
		if err != nil {
...
func (sq *SqliteDB) GetSeedsByUnit(tx *sql.Tx, unit cashu.Unit) ([]Seed, error) {
	seeds := []Seed{}
	stmt, err := tx.Prepare("SELECT  created_at, active, version, unit, id, input_fee_ppk, legacy FROM seeds WHERE unit = $1")
	if err != nil {
		return seeds, fmt.Errorf(`tx.Prepare("SELECT  created_at, active, version, unit, id, input_fee_ppk, legacy FROM seeds WHERE unit = $1"). %w`, err)
	}
	defer stmt.Close()
...
func (sq *SqliteDB) SaveNewSeed(tx *sql.Tx, seed Seed) error {
	...
	for {
		tries += 1
		_, err := tx.Exec("INSERT INTO seeds ( active, created_at, unit, id, version, input_fee_ppk, legacy) VALUES ($1, $2, $3, $4, $5, $6, $7)", seed.Active, seed.CreatedAt, seed.Unit, seed.Id, seed.Version, seed.InputFeePpk, seed.Legacy)
